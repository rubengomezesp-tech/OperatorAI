...
export async function* runChatWithTools(args: RunArgs): AsyncIterable<ToolStreamEvent> {
  ...
  for (let loop = 0; loop < MAX_TOOL_LOOPS; loop++) {
      ...
      for await (const event of stream) {
        if (event.type === 'content_block_start') {
          ...
          if (block.type === 'tool_use') {
            ...
            // Emit tool_start AND a text indicator the user can see
    ...
    for (const use of pendingToolUses) {
      // Show a generating indicator as text
      ...
      // Emit tool_result event (for frontend components that handle it)
      yield {
        type: 'tool_result',
      ...
      // Inject result directly into the text stream so the user sees it
      // Uses markdown syntax that MarkdownBody already renders
      if (execResult.ok) {
      ...
      toolResultBlocks.push({
        ...
        content: execResult.ok
          ? JSON.stringify({ success: true, ...(execResult.result ?? {}), note: 'Result already displayed inline to the user. Do NOT repeat URLs or embed images in your text. Just continue the conversation naturally.' })
