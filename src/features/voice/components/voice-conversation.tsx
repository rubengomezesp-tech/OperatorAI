...
export function VoiceConversation() {
  ...
  const { speak, stop: stopTTS, state: ttsState } = useTTS({
    voice: 'nova',
    onEnd: () => {
      // After assistant finishes speaking, in auto mode start listening again
      if (autoMode && phaseRef.current === 'speaking') {
        setTimeout(() => {
  ...
  // Sync recorder state into our phase
  useEffect(() => {
