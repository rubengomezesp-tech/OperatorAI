...
export class OperatorCoachUnavailableError extends Error {
  constructor(message?: string) {
    super(
      message
        ?? 'OperatorAI coach no esta disponible. Verifica que OPERATOR_COACH_URL responda y que el modelo pueda completar.'
    );
...
function buildUnavailableMessage(probe: Awaited<ReturnType<typeof probeOperatorCoach>>): string {
  const base = `OperatorAI coach no esta disponible en ${probe.config.url} usando modelo ${probe.config.model}.`;
