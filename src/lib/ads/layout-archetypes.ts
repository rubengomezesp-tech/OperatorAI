...
export const ARCHETYPES: Record<ArchetypeId, LayoutArchetype> = {
  ...
  'brand-system-document': {
    ...
    promptDirective:
      'Generate a COMPREHENSIVE BRAND SYSTEM EXPLORATION DOCUMENT — NOT a single ad. This is a multi-section editorial spread on a 12-column grid, designed like a Pentagram brand presentation deck or Apple Human Interface Guidelines page. CRITICAL: USE THE BRAND CONTEXT PROVIDED — extract the ACTUAL brand name, brand colors (HEX codes), typography stack, product/service nature, and tone-of-voice from the brief. DO NOT invent generic placeholders. DO NOT use template character names like "Operador Elite" — instead reference the ACTUAL brand identity. The document MUST contain 6-8 distinct labeled sections in magazine-spread layout: (1) IDENTITY ANCHOR — the brand actual logo + actual tagline + brand essence sentence (top-left quadrant). (2) CONCEPT EXPLORATION — 3 numbered character/product/service variants RELEVANT to this specific brand and product (with descriptors that match the brand voice — NOT generic templates). (3) APPLICATION GRID — 5 icon variations of the actual logo across contexts (dark+brand-color, flat-brand-color, white, white-on-dark, transparent). (4) PRODUCT MOCKUPS — 3 realistic device mockups appropriate to the product type: if app/SaaS show laptop UI + mobile screen; if physical product show product photography + packaging + retail context; if service show real-environment-of-use (gym, kitchen, office, etc.). (5) COLOR SYSTEM — 5 color swatches with the BRAND ACTUAL HEX codes and color names (extracted from brand context, NEVER hardcoded). (6) TYPOGRAPHY STACK — display the BRAND ACTUAL typeface name in massive scale + list the actual font families used by this brand (NEVER hardcoded fonts like SORA unless the brand actually uses them). (7) BRAND ESSENCE — 4 small icons with labels reflecting the BRAND ACTUAL VALUES (extracted from voice/tone — NOT generic words). Feel: high-end Pentagram brand deliverable — editorial density with breathing room, premium gravitas, expert curation. NOT a marketing ad. NOT a single hero shot. This IS a BRAND BOOK PAGE that genuinely represents THIS specific brand.',
    compositionRules: [
